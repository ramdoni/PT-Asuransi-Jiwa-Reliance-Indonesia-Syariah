...
class Index extends Component
{
    ...
    public $check_id=[],$filter_polis_id,$is_download=false,$check_all,$polis;
    ...
    public function mount()
    {
        $this->polis = Polis::select('polis.*')
                            ->join('refund','refund.polis_id','=','polis.id')
                            ->groupBy('polis.id')
                            ->get();
    }
    ...
    public function downloadExcel()
    {
        ...
        $activeSheet->setCellValue('A2', 'DAFTAR REFUND KEPESERTAAN ASURANSI JIWA KUMPULAN SYARIAH');
        ...
        $activeSheet
                ...
                ->setCellValue('H9', 'TGL. EFEKTIF PENGURANGAN')
                ->setCellValue('I9', 'NILAI MANFAAT ASURANSI')
                ->setCellValue('J9', 'PENGEMBALIAN KONTRIBUSI')
                ->setCellValue('K9', 'KONTRIBUSI')
        ...
        $data = Kepesertaan::where('polis_id',$this->filter_polis_id)->whereIn('memo_refund_id',$this->check_id)->get();
        $num=10;
        ...
        foreach($data as $k=>$i){
            $activeSheet
                ...
                ->setCellValue("H{$num}", date('d-M-Y',strtotime($i->refund_tanggal_efektif)))
                ->setCellValue("I{$num}", $i->basic)
                ->setCellValue("J{$num}", $i->refund_kontribusi)
