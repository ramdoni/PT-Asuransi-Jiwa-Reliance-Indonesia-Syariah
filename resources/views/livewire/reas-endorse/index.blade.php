                    ...
                    <table class="table table-hover m-b-0 c_list table-nowrap" id="data_table">
                        <thead style="vertical-align:middle">
                            <tr>
                                ...
                                <th class="text-center">Kontribusi Refund</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($data as $k => $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('endorsement.edit', $item->endorsement_id)}}" target="_blank">{{isset($item->endorsement->no_pengajuan) ? $item->endorsement->no_pengajuan : '-'}}</a>
                                    </td>
                                    <td>
                                        {{isset($item->reas->reasuradur->name) ? $item->reas->reasuradur->name : '-'}}
                                    </td>
                                    <td>
                                        <a href="{{route('reas-endorse.edit', $item->id)}}">{{$item->nomor}}</a>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-right">{{format_idr($item->total_kontribusi_refund)}}</td>
                                </tr>
                            @endforeach
                            @if($data->count()==0)
